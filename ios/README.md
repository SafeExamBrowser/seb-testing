...
**Testing especially this beta version with as many exam systems as possible is crucial, because it uses some custom networking and web request handling code (a custom web cache and when the Browser Exam/Config Key or embedded SSL certificates are used, a custom URL loading protocol). So the more feedback from SEB users we receive, the earlier we can release it.**

As the SEB for iOS app is updated automatically on student devices as soon as a new version is available in the App Store, **it's important that SEB power users test it and report issues BEFORE the final version is released**. So if you are using SEB for iOS in your institution or with your assessment solution, please contribute to the SEB open source freeware project by participating in our beta version testing.
